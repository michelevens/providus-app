...
const SERVICE_LINES = [
  ...
  {
    id: 'hra-awv',
    name: 'At-Home & Telehealth Health Risk Assessments (HRA/AWV)',
    status: 'planned',
    icon: '&#127969;',
    color: '#0ea5e9',
    summary: 'Contracted per-visit Annual Wellness Visits and Health Risk Assessments for Medicare Advantage plans, ACOs, and delegated risk groups — delivered via telehealth (and optionally in-home) by NPs. Competes with HarmonyCares, Signify Health, Matrix Medical, and Landmark.',
    targetPatient: 'Medicare Advantage members, ACO-attributed Medicare beneficiaries, Dual-Eligible (MMP) members, and high-risk complex patients who need annual HRA/AWV visits to close HEDIS quality gaps and capture HCC risk adjustment coding. Typically 65+, often with 3+ chronic conditions.',
    revenueModel: 'Per-visit contracted rate with health plans/ACOs ($85-$145/visit). NOT fee-for-service billing — the plan pays a flat rate for a completed compliant HRA/AWV. Optional PMPM (per-member-per-month) model for dedicated panel arrangements. High-volume, low-acuity, scalable model.',
    annualRevenuePerPatient: '$85 - $145 per completed visit (typically 1 visit/year/member)',
    visitFrequency: 'Annual (once per calendar year per member, per CMS rules)',
    billingCodes: [
      { code: 'G0438', desc: 'Initial Medicare AWV (first ever AWV)', rate: '$150 - $180 (if billing FFS) / $85-$145 (contracted rate)' },
      { code: 'G0439', desc: 'Subsequent Medicare AWV (annual, after first AWV)', rate: '$110 - $140 (FFS) / $85-$145 (contracted)' },
      { code: 'G0402', desc: 'Initial Preventive Physical Exam (IPPE, Welcome to Medicare)', rate: '$140 - $165' },
      { code: '99497', desc: 'Advance care planning, first 30 min (add-on to AWV)', rate: '$80 - $100' },
      { code: '99498', desc: 'Advance care planning, each additional 30 min', rate: '$70 - $90' },
      { code: 'G0444', desc: 'Annual depression screening (PHQ-9)', rate: '$18 - $25' },
      { code: 'G0442', desc: 'Annual alcohol misuse screening', rate: '$18 - $25' },
      { code: 'G0443', desc: 'Brief alcohol misuse counseling', rate: '$25 - $30' },
      { code: '96160', desc: 'Health risk assessment administration (patient-focused)', rate: '$8 - $12' },
      { code: '96161', desc: 'Caregiver-focused HRA (for dementia patients)', rate: '$8 - $12' },
      { code: 'G0296', desc: 'Lung cancer screening counseling (if indicated)', rate: '$25 - $35' },
      { code: 'HCC', desc: 'HCC Risk Adjustment coding capture (drives plan revenue — key value)', rate: 'Value: $400-$3,000/HCC to plan' },
    ],
    clinicalConsiderations: [
      'HRA/AWV is NOT a problem-focused visit — do NOT treat acute issues; refer back to PCP. Separate E/M can be billed only with modifier 25 and clear documentation.',
      'NCQA-compliant documentation required — each question in the HRA template must be answered and timestamped',
      'HCC (Hierarchical Condition Category) coding drives plan revenue — capture ALL chronic conditions with MEAT criteria (Monitored, Evaluated, Assessed, Treated) per visit',
      'Required elements: functional status (ADLs/IADLs), fall risk screen, cognitive screen (Mini-Cog or MoCA), depression screen (PHQ-2/9), advance care planning offer, USPSTF preventive care gaps',
      'Telehealth AWVs permitted by CMS (post-PHE extended through 2025+) — confirm each plan allows telehealth AWV in their contract',
      'Plan will require specific templates/portals (e.g., Inovalon, Episource, Change Healthcare) — each vendor has own workflow',
      'NP scope: AWVs are fully within NP scope in all states; no physician co-signature required in full-practice states',
      'Medicare requires member consent for AWV (document) and only ONE AWV per 12 months (G0438 once lifetime, then G0439 annually)',
      'For in-home visits: OSHA bloodborne pathogen training, home safety protocols, 2-person visits for high-risk members if required by plan',
      'Quality metrics plans watch: completion rate, turnaround time (referral to visit <30 days), HCC capture per visit (target 3-5 HCCs), member satisfaction NPS (>70)',
    ],
    credentialingNotes: 'No special HRA/AWV credentialing — standard NP licensure + Medicare enrollment is the prerequisite. The real gate is the PAYER CONTRACT. Must be enrolled with Medicare (PTAN) + credentialed with each contracting MA plan OR the vendor (Signify, Episource, Inovalon) who subcontracts the work. Contract types: (1) Direct MA plan contract (highest margin, hardest to get), (2) Subcontractor/vendor model (Signify, Matrix — faster to start, lower margin 50-60% of posted rate), (3) ACO delegated arrangement (medium margin, good volume). Avg contract cycle: 90-180 days for direct plan, 30-60 days for vendor subcontract.',
    marketDemand: 'Massive and growing. 33M+ Medicare Advantage members (52% of all Medicare, growing ~2M/year). MA plans spend $4-6B annually on in-home/telehealth HRA vendors. Major players: Signify Health (acquired by CVS for $8B in 2023), HarmonyCares, Matrix Medical Network, Landmark Health, Episource. CMS is tightening risk-adjustment rules (V28 HCC model rollout 2024-2026), increasing plans\' urgency to complete compliant HRAs. Provider shortage + aging population + HEDIS/Stars bonus pressure = durable tailwind. EnnHealth\'s unique edge: existing telehealth infrastructure + 20-state NP licensing + behavioral health integration (depression/anxiety screens are top HEDIS gaps — you can close BOTH medical and behavioral gaps in one visit, which NO competitor offers).',
    outreachContacts: [
      { tier: 'Vendor Subcontract (Fast Start)', payer: 'Signify Health (CVS)', contact: 'provider-recruitment@signifyhealth.com | signifyhealth.com/clinicians', status: 'planned' },
      { tier: 'Vendor Subcontract (Fast Start)', payer: 'Matrix Medical Network', contact: 'clinicianrecruiting@matrixhealth.net | matrixhealth.net', status: 'planned' },
      { tier: 'Vendor Subcontract (Fast Start)', payer: 'Episource', contact: 'provider@episource.com | episource.com', status: 'planned' },
      { tier: 'Vendor Subcontract (Fast Start)', payer: 'Inovalon', contact: 'providerrelations@inovalon.com', status: 'planned' },
      { tier: 'Vendor Subcontract (Fast Start)', payer: 'HarmonyCares', contact: 'careers.harmonycares.com (PRN NP model)', status: 'planned' },
      { tier: 'Direct MA Plan', payer: 'Humana (largest MA plan)', contact: '800-448-6262 | risk-adjustment team', status: 'planned' },
      { tier: 'Direct MA Plan', payer: 'UnitedHealthcare MA (Optum)', contact: 'already credentialed — escalate to HouseCalls program', status: 'outreach' },
      { tier: 'Direct MA Plan', payer: 'Aetna MA (CVS)', contact: '800-624-0756 | risk-adjustment team', status: 'planned' },
      { tier: 'Direct MA Plan', payer: 'Cigna Medicare', contact: '800-230-6138', status: 'planned' },
      { tier: 'Direct MA Plan', payer: 'Anthem/Elevance MA', contact: '800-676-2583', status: 'planned' },
      { tier: 'Direct MA Plan', payer: 'Florida Blue Medicare', contact: 'already partnered via Lucet — ask about AWV program', status: 'outreach' },
      { tier: 'ACO / Risk Groups', payer: 'Aledade (primary care ACO network)', contact: 'providerrelations@aledade.com', status: 'planned' },
      { tier: 'ACO / Risk Groups', payer: 'Agilon Health', contact: 'providerrelations@agilonhealth.com', status: 'planned' },
      { tier: 'ACO / Risk Groups', payer: 'CareMore / Elevance', contact: 'Through Anthem/Elevance', status: 'planned' },
      { tier: 'ACO / Risk Groups', payer: 'Oak Street Health', contact: 'providerrelations@oakstreethealth.com', status: 'planned' },
      { tier: 'ACO / Risk Groups', payer: 'ChenMed', contact: 'providerrelations@chenmed.com', status: 'planned' },
    ],
  },
];
...
const SERVICE_LINE_INTEL = {
  ...
  'hra-awv': {
    businessCase: [
      'PER-VISIT ECONOMICS: $85-$145/visit × 8 visits/day × 20 days/month = $13,600-$23,200/month per NP. NP pay at 50-60% of visit fee (PRN 1099 model) = $6,800-$13,920 to NP, $6,800-$9,280 gross margin per NP per month.',
      'SCALE ECONOMICS: 10 PRN NPs × $8,000 avg monthly gross margin = $80,000/month = $960,000/year. 25 PRN NPs = $2.4M/year. Infrastructure cost (scheduling, QA, platform) flat ~$30k/month — massive operating leverage.',
      'NO PATIENT ACQUISITION COST: The health plan finds and schedules the patient. You just show up (virtually or in-home) and complete the visit. 100% utilization when contracts are flowing.',
      'RECURRING ANNUAL REVENUE: Each member = 1 visit/year. Retain a 5,000-member panel = 5,000 visits/year × $100 avg = $500,000/year guaranteed annual revenue per contract.',
      'HCC VALUE CAPTURE: Each HCC captured is worth $400-$3,000 to the plan (in CMS risk-adjustment payments). Plans willing to pay premium for providers who capture 3-5 HCCs per visit vs 0-1. Negotiate rate based on HCC capture performance.',
      'BEHAVIORAL HEALTH DIFFERENTIATOR: No competitor (Signify, Matrix, HarmonyCares, Landmark) has integrated BH. You can close both medical gaps (diabetes, BP, cancer screens) AND behavioral gaps (depression, anxiety, SUD) in one visit. This is worth 20-40% premium to plans because behavioral gaps are the hardest to close and most penalized in Stars ratings.',
      'ACQUISITION/EXIT VALUE: Signify sold for $8B (15x revenue). Matrix Medical valued at $500M+. A profitable HRA/AWV practice with diversified payer mix is an acquisition target for health plans, vendors, and PE.',
      'BUILDS PATIENT PIPELINE: Every AWV is a screening for psych, weight, AUD, chronic care needs. Patients needing follow-up get referred back to their PCP OR — with patient consent — to EnnHealth\'s psych/weight/MAT service lines. HRA becomes a feeder for other service lines.',
    ],
    codingGuide: [
      'CORE CODES: G0438 (initial AWV, once per lifetime), G0439 (subsequent AWV, annual thereafter), G0402 (IPPE/Welcome to Medicare visit, first 12 months of Part B). Never bill both G0438 and G0402 for same patient.',
      'ADD-ONS THAT DRIVE REVENUE: 99497 (Advance Care Planning, +$80-100) — always offer, ~30% of patients accept. G0444 (depression screen, +$18-25) — required by CMS, always bill. G0442 (alcohol screen, +$18-25) — required, always bill. Combined add-ons can add $120-150 to base visit.',
      '96160/96161 HRA ADMINISTRATION: billable for HRA tool administration separately from AWV. Adds $8-12. Often missed.',
      'HCC CAPTURE IS THE VALUE: even though you are paid per visit (not FFS), the plan\'s ROI on YOU is driven by HCC capture. Document every chronic condition with MEAT (Monitored/Evaluated/Assessed/Treated). Common high-value HCCs: major depression (HCC 155, factor 0.309), COPD (HCC 111, factor 0.335), diabetes w/ complications (HCC 18, factor 0.302), CKD stage 3+ (HCC 138, factor 0.289), morbid obesity with BMI>35 (HCC 22, factor 0.250).',
      'TELEHEALTH MODIFIERS: AWV via telehealth uses POS 10 (patient at home) + modifier 95 (synchronous audiovisual). If audio-only allowed by plan, use modifier 93/FQ. Confirm each plan\'s telehealth AWV policy.',
      'DO NOT BILL PROBLEM-FOCUSED E/M WITH AWV unless modifier 25 is justified. The AWV is preventive only. Acute issues discovered during AWV should be documented and referred to PCP — not treated or billed as separate E/M unless clearly medically necessary, separately identifiable, and documented.',
      'SEPARATE MODIFIERS BY PAYER: Medicare FFS uses G-codes directly. MA plans may accept G-codes OR require their own HCPCS/CPT (check contract). Some plans (Humana, UHC) have proprietary HRA codes.',
      'AVOID DENIALS: (1) Member age ≥65 and has Part B, (2) 12+ months since last AWV (G0439), (3) first-ever AWV must be G0438 not G0439, (4) document ALL 10 required AWV elements or visit is non-billable.',
    ],
    clinicalProtocol: [
      'REQUIRED AWV ELEMENTS (CMS): (1) Medical/family/social history review, (2) measurement of vital signs (BMI, BP), (3) cognitive assessment (Mini-Cog or MoCA), (4) functional ability & safety (ADLs/IADLs, fall risk, home safety), (5) depression screening (PHQ-2 → PHQ-9 if positive), (6) written screening schedule next 5-10 years, (7) list of risk factors/conditions, (8) personalized health advice, (9) voluntary advance care planning offer, (10) end-of-life preferences discussion.',
      'INTAKE WORKFLOW: Plan sends member list → your scheduling team calls/texts member → patient completes pre-visit HRA questionnaire (online or paper) → virtual or in-home visit → NP conducts AWV (45-60 min) → documentation in plan\'s portal within 24-48 hours → plan QA reviews → payment cycle (net-30 typical).',
      'DOCUMENTATION STANDARD: Plans audit 10-20% of visits. Template must capture: all 10 AWV elements, each HCC diagnosis with MEAT, screening results (PHQ-9 score, MoCA score, fall risk score), referrals back to PCP, next steps. Incomplete documentation = clawback + lost contract.',
      'HCC COMPLETENESS: Build HCC checklist into workflow. Review chart BEFORE visit for suspected HCCs. Confirm each condition is still active. If new HCC identified, document MEAT, ICD-10, and clinical rationale. Avg high-performer captures 4-5 HCCs/visit vs low-performer 1-2.',
      'QUALITY METRICS TRACKED BY PLAN: Visit completion rate (>80%), member satisfaction NPS (>70), time-to-visit from referral (<30 days), no-show rate (<15%), documentation audit pass rate (>95%), HCC recapture rate (>70% of prior year conditions).',
      'TELEHEALTH VS IN-HOME TRIAGE: Phone/video triage call determines modality. Telehealth OK for: mobile patients, tech-capable, no fall/safety concerns. In-home required for: severely frail, cognitive impairment, poor technology literacy, urgent safety concerns, high-risk meds requiring visual check.',
      'POST-VISIT REFERRALS: After AWV, send findings to PCP within 7 days (fax, portal, direct message). Include: HCC list, care gaps identified, referral recommendations, patient education provided. Good PCP communication = plan loyalty.',
      'NCQA/HEDIS FOCUS GAPS: plans care most about closing: annual wellness visit completion, diabetes A1c<9, BP control (<140/90), breast/colon/cervical cancer screens, statin therapy, depression screen + follow-up, fall risk intervention, osteoporosis mgmt, medication reconciliation post-discharge.',
    ],
    launchChecklist: [
      'STEP 1 — ENSURE MEDICARE ENROLLMENT: confirm PTAN active for all NPs. If not enrolled, enroll via PECOS (45-60 days). Required before any MA plan contract.',
      'STEP 2 — INVENTORY CURRENT MA PLAN CONTRACTS: list all MA plans EnnHealth is already credentialed with. These are lowest-friction starting points — ask each for their HRA/AWV or HouseCalls program. UHC has HouseCalls, Humana has Transcend, Aetna has "In-Home Health Visit" program.',
      'STEP 3 — APPLY TO VENDOR SUBCONTRACT NETWORKS (FAST START): Signify Health, Matrix Medical, Episource, Inovalon all onboard PRN NPs via portal. 30-60 day onboarding. Lower margin (50-60% of visit fee) but fastest path to first dollar. Start with 1-2 vendors.',
      'STEP 4 — BUILD HRA/AWV DOCUMENTATION TEMPLATE: work with dev team to create structured template in Credentik/EHR covering all 10 CMS AWV elements + HCC capture prompts + PHQ-9/MoCA/fall risk screens + export to plan portals (Inovalon, Episource formats).',
      'STEP 5 — HIRE OR DESIGNATE PRN NP POOL: recruit 2-3 PRN NPs (1099) with geriatric experience. Compensation: $50-$75/visit (50-60% of contract rate). Target 6-8 visits/day each. Use Credentik to credential them fast.',
      'STEP 6 — TRAIN ON HCC CODING: mandatory 4-hour CDI (clinical documentation improvement) course. Use free AAPC/AHIMA materials or hire HCC coding consultant. Review first 20 charts per NP for coding accuracy.',
      'STEP 7 — PILOT 50-100 VISITS: accept first vendor subcontract. Run 50-100 visits. Measure: completion rate, HCC capture, QA pass rate, NP productivity. Iterate workflow.',
      'STEP 8 — DIRECT MA PLAN OUTREACH: with vendor-subcontract track record, approach MA plan risk-adjustment teams directly. Pitch: "We completed [X] HRAs with [Y] HCC capture rate at [Z] quality score. Can we negotiate a direct contract at higher rate?" Direct contracts = 2x margin of vendor.',
      'STEP 9 — ACO PARTNERSHIPS: target Aledade, Agilon, Oak Street, ChenMed — they manage Medicare attributed lives and need HRA completion to hit shared savings targets. ACOs often pay highest rates ($120-150/visit) because HRA drives their risk-adjustment revenue.',
      'STEP 10 — BEHAVIORAL HEALTH PITCH: once basic HRA flow is stable, pitch BH-integrated HRA to plans as premium product. "We close medical + behavioral care gaps in one visit." Negotiate 20-40% premium. This is the moat — no competitor offers this.',
      'STEP 11 — SCALE NP POOL: as contracts grow, scale PRN pool. Target 10-25 NPs in 12-18 months. Use Credentik for fast multi-state licensing/credentialing. Aim for NP pool covering all 20 states EnnHealth is licensed in.',
      'STEP 12 — QA INFRASTRUCTURE: hire or designate a clinical QA lead to audit 10-20% of visits monthly. Plans will audit — better to catch issues internally first. Maintain >95% documentation pass rate.',
      'KEY TOOLS NEEDED: HRA template in EHR, scheduling system with time-block for HRAs, PCP fax/portal integration for post-visit reports, HCC coding tool (or trained coder), pre-visit patient questionnaire (e.g., DocuSign/Google Forms), telehealth video platform (already exists), mobile checklist app for in-home visits.',
      'TEMPLATE EMAIL TO MA PLAN/VENDOR: "I am [Name], [Credential] with EnnHealth Medical Group (NPI: 1861107849). We are a multi-state telehealth practice licensed in 20+ states with PMHNP and FNP providers. We are interested in contracting as an HRA/AWV vendor for your Medicare Advantage population. We offer a unique behavioral-health-integrated HRA model — we close both medical and behavioral care gaps in a single visit, which improves HEDIS and Stars performance. Available for a 30-minute call to discuss contracting options. [Contact info]"',
    ],
  },
};
